...
class Command(BaseCommand):
    ...
    def train_ml_model(self, x_train, y_train, x_test, y_test, save_path, chamber):
        ...
        if chamber == 'house':
            ...
            # model = DecisionTreeClassifier(max_depth=6, max_features=4)
        else:
            # model = DecisionTreeClassifier(min_samples_split=2)
            # model = GradientBoostingClassifier(n_estimators=25, max_features=4, learning_rate=0.01)
            # model = LogisticRegression(max_iter=500, fit_intercept=False)
            model = ComplementNB()
    ...
    def handle(self, *args, **options):
        ...
        if options['use_feature_cache']:
            ...
            # house_amendments = Bill.objects.filter(status__in=['pass', 'fail'], bill_id__startswith='h').exclude(bill_id__endswith='117')
            ...
            senate_bills = Bill.objects.filter(status__in=SENATE_PASS_STATUS_LIST + SENATE_FAIL_STATUS_LIST,
                                               introduced_at__year__gte=2014).exclude(bill_id__endswith='117')
            # senate_amendments = Bill.objects.filter(status__in=['pass', 'fail'], bill_id__startswith='s').exclude(bill_id__endswith='117')
